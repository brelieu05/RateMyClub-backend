...
const uploadMultipartWithProgress = (file, presigned, opts)=>Micro.gen(function*() {
        ...
        const etags = yield* Micro.forEach(presigned.urls, (url, index)=>{
            ...
            concurrency: "inherit"
        }).pipe(Micro.tapExpected((error)=>opts.reportEventToUT("failure", {
 ...
 */ const genUploader = (initOpts)=>{
    return (endpoint, opts)=>uploadFilesInternal(endpoint, {
        ...
        }), (e)=>Micro.runPromiseResult(e, opts.signal ? {
                signal: opts.signal
            } : {})).then((result)=>{
...
const uploadFilesInternal = (endpoint, opts)=>{
    ...
    return reportEventToUT("upload", {
        ...
        files: opts.files.map((f)=>({
                ...
                ...opts,
                reportEventToUT
            }, presigned).pipe(Micro.onAbort(reportEventToUT("failure", {
                fileKey: presigned.key,
...
const uploadFile = (slug, opts, presigned)=>Arr.findFirst(opts.files, (file)=>file.name === presigned.fileName).pipe(Micro.fromOption, Micro.mapError(()=>{
        ...
        headers: {
            ...
            message: e.message,
            cause: e
        }))), Micro.filterOrFailWith(isPollingResponse, (_)=>Micro.FailureUnexpected("received a non PollingResponse from the polling endpoint")), Micro.filterOrFail(isPollingDone, ()=>new RetryError()), Micro.map(({ callbackData })=>callbackData), Micro.retry({
        while: (res)=>{
