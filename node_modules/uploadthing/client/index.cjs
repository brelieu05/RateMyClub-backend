...
const uploadMultipartWithProgress = (file, presigned, opts)=>Micro__namespace.gen(function*() {
        ...
        const etags = yield* Micro__namespace.forEach(presigned.urls, (url, index)=>{
            ...
            }).pipe(Micro__namespace.andThen((tag)=>({
                    tag,
                    partNumber: index + 1
                })), Micro__namespace.retry({
                while: (error)=>error instanceof shared.RetryError,
                times: stdEnv.isTest ? 3 : 10,
                delay: shared.exponentialDelay()
            }));
        }, {
            concurrency: "inherit"
        }).pipe(Micro__namespace.tapExpected((error)=>opts.reportEventToUT("failure", {
 ...
 */ const genUploader = (initOpts)=>{
    return (endpoint, opts)=>uploadFilesInternal(endpoint, {
        ...
        }), (e)=>Micro__namespace.runPromiseResult(e, opts.signal ? {
                signal: opts.signal
            } : {})).then((result)=>{
            ...
            throw Micro__namespace.failureSquash(result.left);
        });
};
const uploadFilesInternal = (endpoint, opts)=>{
    ...
    return reportEventToUT("upload", {
        ...
        files: opts.files.map((f)=>({
                ...
                ...opts,
                reportEventToUT
            }, presigned).pipe(Micro__namespace.onAbort(reportEventToUT("failure", {
                fileKey: presigned.key,
...
const uploadFile = (slug, opts, presigned)=>Arr__namespace.findFirst(opts.files, (file)=>file.name === presigned.fileName).pipe(Micro__namespace.fromOption, Micro__namespace.mapError(()=>{
        ...
        headers: {
            ...
            message: e.message,
            cause: e
        }))), Micro__namespace.filterOrFailWith(isPollingResponse, (_)=>Micro__namespace.FailureUnexpected("received a non PollingResponse from the polling endpoint")), Micro__namespace.filterOrFail(isPollingDone, ()=>new shared.RetryError()), Micro__namespace.map(({ callbackData })=>callbackData), Micro__namespace.retry({
        while: (res)=>{
            return res instanceof shared.RetryError;
        },
        delay: shared.exponentialDelay()
