...
const conditionalDevServer = (fileKey, apiKey)=>{
    return Effect.gen(function*() {
        const file = yield* fetchEff(generateUploadThingURL(`/api/pollUpload/${fileKey}`)).pipe(Effect.andThen(parseResponseJson), Effect.andThen(S.decodeUnknown(PollUploadResponse)), Effect.andThen((res)=>res.status === "done" && res.fileData ? Effect.succeed(res.fileData) : Effect.fail(new RetryError())), Effect.retry({
...
const handleUploadAction = Effect.gen(function*() {
    ...
    if (opts.isDev) {
        ...
        promise = Effect.forEach(presignedUrls, (file)=>conditionalDevServer(file.key, opts.apiKey).pipe(Effect.either), {
            concurrency: 10
...
const uploadFile = (input)=>Effect.gen(function*() {
        ...
        yield* fetchEff(generateUploadThingURL(`/api/pollUpload/${presigned.key}`)).pipe(Effect.andThen(parseResponseJson), Effect.andThen(S.decodeUnknown(PollUploadResponse)), Effect.tap(Effect.logDebug("Polled upload", presigned.key)), Effect.andThen((res)=>res.status === "done" ? Effect.succeed(undefined) : Effect.fail(new RetryError())), Effect.retry({
            while: (err)=>err instanceof RetryError,
            schedule: Schedule.exponential(Duration.millis(10), 4).pipe(// 10ms, 40ms, 160ms, 640ms...
            Schedule.andThenEither(Schedule.spaced(Duration.seconds(1))), Schedule.compose(Schedule.elapsed), Schedule.whileOutput(Duration.lessThanOrEqualTo(Duration.minutes(1))))
        }), Effect.catchTag("RetryError", (e)=>Effect.die(e)));
